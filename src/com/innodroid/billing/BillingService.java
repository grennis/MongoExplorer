...
import com.innodroid.billing.Security.VerifiedPurchase;
...
public class BillingService extends Service implements ServiceConnection {
    ...
    abstract class BillingRequest {
        ...
        public boolean runIfConnected() {
            if (Consts.DEBUG) {
                Log.d(Constants.LOG_TAG, getClass().getSimpleName());
            }
            if (mService != null) {
                try {
                    mRequestId = run();
                    if (Consts.DEBUG) {
                        Log.d(Constants.LOG_TAG, "request id: " + mRequestId);
                    }
                    if (mRequestId >= 0) {
        ...
        protected void onRemoteException(RemoteException e) {
            Log.w(Constants.LOG_TAG, "remote billing service crashed");
            mService = null;
        }
        ...
        protected void logResponseCode(String method, Bundle response) {
            ...
            if (Consts.DEBUG) {
                Log.e(Constants.LOG_TAG, method + " received " + responseCode.toString());
            }
        }
    }
    ...
    public class CheckBillingSupported extends BillingRequest {
        ...
        protected long run() throws RemoteException {
            ...
            if (Consts.DEBUG) {
                Log.i(Constants.LOG_TAG, "CheckBillingSupported response code: " +
    ...
    public class RequestPurchase extends BillingRequest {
        ...
        protected long run() throws RemoteException {
            ...
            if (pendingIntent == null) {
                Log.e(Constants.LOG_TAG, "Error with requestPurchase");
                return Consts.BILLING_RESPONSE_INVALID_REQUEST_ID;
            }
    ...
    public void handleCommand(Intent intent, int startId) {
        ...
        if (Consts.DEBUG) {
            Log.i(Constants.LOG_TAG, "handleCommand() action: " + action);
        }
    ...
    private boolean bindToMarketBillingService() {
        try {
            if (Consts.DEBUG) {
                Log.i(Constants.LOG_TAG, "binding to Market billing service");
            }
            ...
            if (bindResult) {
                ...
                Log.e(Constants.LOG_TAG, "Could not bind to service.");
            }
        } catch (SecurityException e) {
            Log.e(Constants.LOG_TAG, "Security exception: " + e);
        }
        return false;
    }
    ...
    private void checkResponseCode(long requestId, ResponseCode responseCode) {
        ...
        if (request != null) {
            if (Consts.DEBUG) {
                Log.d(Constants.LOG_TAG, request.getClass().getSimpleName() + ": " + responseCode);
            }
    ...
    private void runPendingRequests() {
        ...
        if (maxStartId >= 0) {
            if (Consts.DEBUG) {
                Log.i(Constants.LOG_TAG, "stopping service, startId: " + maxStartId);
            }
            stopSelf(maxStartId);
        }
    }
    ...
    public void onServiceConnected(ComponentName name, IBinder service) {
        if (Consts.DEBUG) {
            Log.d(Constants.LOG_TAG, "Billing service connected");
        }
    ...
    public void onServiceDisconnected(ComponentName name) {
        Log.w(Constants.LOG_TAG, "Billing service disconnected");
        mService = null;
    }
